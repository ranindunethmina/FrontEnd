...
async function populateCustomerDropdown() {
  ...
  try {
    const response = await fetch("http://localhost:8080/WebPosSpring/api/v1/customer");
    if (!response.ok) {
...
async function populateOrderItems() {
  try {
    const response = await fetch("http://localhost:8080/WebPosSpring/api/v1/item");
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
...
async function loadOrderId() {
  try {
    const response = await fetch("http://localhost:8080/WebPosSpring/api/v1/order");
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
